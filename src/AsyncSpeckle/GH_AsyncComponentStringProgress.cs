...
namespace Brain.GrasshopperAsyncComponent
{
    ...
    /// </summary>
    public abstract class GH_AsyncComponentStringProgress : GH_Component
    {
        ...
        public List<WorkerInstance<string>> Workers;
        ...
        /// </summary>
        public WorkerInstance<string> BaseWorker { get; set; }
        ...
        protected GH_AsyncComponentStringProgress(string name, string nickname, string description, string category, string subCategory) : base(name, nickname, description, category, subCategory)
        {

            DisplayProgressTimer = new Timer(333) { AutoReset = false };
            DisplayProgressTimer.Elapsed += DisplayProgress;

            ReportProgress = (id, message) =>
            {
                ProgressReports[id] = message;
            ...
            Workers = new List<WorkerInstance<string>>();
        ...
        public virtual void DisplayProgress(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (Workers.Count == 1)
            {
                Message = ProgressReports.Values.Last();
            }
            else
            {
                //double total = 0;
                ...
                var msgs = ProgressReports.Values.ToList();
                string concat = "";
                int len = msgs.Count;
                for (int i = 0; i < len; i++)
                {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (State == 0)
            {
                ...
                var currentRun = TaskCreationOptions != null
                  ? new Task(() => currentWorker.DoWork(ReportProgress, Done, this), tokenSource.Token, (TaskCreationOptions)TaskCreationOptions)
                  : new Task(() => currentWorker.DoWork(ReportProgress, Done, this), tokenSource.Token);
